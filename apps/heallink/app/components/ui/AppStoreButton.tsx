...
export default function AppStoreButtons({
  ...
  const getButtonStyles = (buttonName: string) => {
    ...
    let baseClasses =
      "flex items-center gap-2 rounded-xl px-6 py-3 transition-all";
    let shadowClasses = isPressed ? "neumorph-pressed" : "neumorph-flat";

    // Light theme needs a solid background for contrast
    if (theme === "light") {
      return `${baseClasses} ${shadowClasses} bg-purple-heart/90 text-white`;
    } else {
      return `${baseClasses} ${shadowClasses} text-white`;
    }
  };
