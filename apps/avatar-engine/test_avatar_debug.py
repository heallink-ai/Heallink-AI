...
async def test_glb_loading():
    ...
    # Direct GLB inspection
    try:
        import trimesh
        print(f"🔬 Inspecting GLB structure directly...")
        scene = trimesh.load(str(glb_path))
        
        if isinstance(scene, trimesh.Scene):
            print(f"📊 GLB Scene contains {len(scene.geometry)} geometries:")
            for name, mesh in scene.geometry.items():
                if hasattr(mesh, 'vertices') and hasattr(mesh, 'faces'):
                    print(f"  - {name}: {len(mesh.vertices)} vertices, {len(mesh.faces)} faces")
                else:
                    print(f"  - {name}: Not a mesh (type: {type(mesh)})")
            
            # Check if there are node transforms
            if hasattr(scene, 'graph') and scene.graph:
                print(f"📈 Scene graph nodes: {list(scene.graph.nodes_geometry)}")
        else:
            print(f"📄 GLB is a single mesh: {type(scene)}")
            if hasattr(scene, 'vertices') and hasattr(scene, 'faces'):
                print(f"  - {len(scene.vertices)} vertices, {len(scene.faces)} faces")
    except Exception as e:
        print(f"❌ Direct GLB inspection failed: {e}")
...
async def test_avatar_rendering():
    ...
    # Create config with debug DISABLED for solid rendering
    config = AvatarConfig()
    config.debug = False
    config.debug_wireframe = False
    config.debug_vertices = False
    ...
    if frame is not None:
        ...
        # Save debug frame
        output_path = "debug_avatar_solid_frame.png"
...
async def main():
    ...
    try:
        ...
        # Test 2: Full rendering ONLY (skip face rendering debug to avoid wireframes)
        frame = await test_avatar_rendering()
        
        # Test 3: Individual face rendering (DISABLED for solid rendering test)
