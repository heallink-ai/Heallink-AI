...
export default function InputField({
  ...
  return (
    <div className="mb-4">
      ...
      <div
        className={`relative rounded-xl transition-all duration-300 bg-[color:var(--input)] ${
          focused
        ...
        <div className="flex items-center">
          ...
          {isPassword && (
            <button
              ...
              className="absolute right-4 top-1/2 -translate-y-1/2 text-[color:var(--muted-foreground)] hover:text-[color:var(--foreground)]"
            >
              {passwordVisible ? (
                <svg
      ...
      {error && (
        <p className="text-sm text-[color:var(--error)] mt-1">{error}</p>
      )}
    </div>
  );
}
