...
import { fetchDoctors } from "@/app/api/doctor-api";
...
export default function DoctorsPage() {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["doctors", currentPage, searchTerm],
    queryFn: () => fetchDoctors(currentPage, itemsPerPage, searchTerm),
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {doctors.map((doctor) => (
              <DoctorCard
                key={doctor.id}
                doctor={doctor}
                onEdit={(id) => router.push(`/dashboard/doctors/${id}/edit`)}
                onView={(id) => router.push(`/dashboard/doctors/${id}`)}
              />
            ))}
          </div>
